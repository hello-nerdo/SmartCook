...
export async function GET(
  ...
  try {
    ...
    const recipe = await db.prepare('SELECT * FROM recipes WHERE id = ? AND userId = ?')
      .bind(id, userId)
      .first();
...
export async function DELETE(
  ...
  try {
    ...
    const recipe = await db.prepare('SELECT id FROM recipes WHERE id = ? AND userId = ?')
      .bind(id, userId)
      .first();
    ...
    // Delete the recipe
    await db.prepare('DELETE FROM recipes WHERE id = ? AND userId = ?')
      .bind(id, userId)
      .run();
